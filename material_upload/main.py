...
def get_experiment(experiment_name, group, col, api):
    ...
    try:
        ...
        # Fetch inventory from the DB if it already exists
        experiment = api.get(cript.Experiment, {"name": experiment.name, "collection": col.uid, "group":group.uid}, max_level=0)
        print(f"Found existing experiment: {experiment.name}")

    return experiment

def create_data(value,group,api,exp):
    """Takes a file and associates it with a dataset. Both the file and the data are uploaded.
    @params:
    value:str, file type and path separated by a ; ex: nmr; C:User\file.txt
    group: cript group object
    api: cript api object
    ...
    path=val_list[1].strip()
    data=cript.Data(**{"group":group,"public":True,"name":f'{data_type}_data',"type":data_type,"experiment":exp})
    api.save(data)
    
    file=cript.File(**{"name":f'{data_type}_file',"source":path,"type":"data","data":[data],"group":group})
    api.save(file)
    return [data]
...
def add_properties(material,row,group,api,exp):
    ...
    for header in ["phase","color","mw_n","density","temp_boiling","heat_combustion_mass:max"]:
        ...
        #Tries to create other properties if they have a value entered in the template      
            ...
            if header=="mw_n":
                ...
                if isinstance(row["mw_n:data"][0],str):
                    data=create_data(row["mw_n:data"][0],group,api,exp)
                if isinstance(row["mw_n:method"][0],str):
                    method=row["mw_n:method"][0]
                property_dict={"key":"mw_n","value":val,"data":data,"method":method, "unit":units}
            ...
            #An example of a property with a condition for its measurement    
                ...
                if not math.isnan(cond_val):
                    ...
                    #in a list
                    condition=[cript.Condition(**{"key":"temperature","type":"value","value":cond_val,"unit":cond_units})]
                
                property_dict={"key":"density","value":val,"unit":units,"conditions":condition}
            ...
            #Example of a property with an uncertainty
                ...
                uncert_type="stdev"
                property_dict={"key":"temp_boiling","value":val,"uncertainty":uncert_val,"uncertainty_type":uncert_type, "unit":units}
                
            #Example of a property that is relative i.e. max, median, mode, etc.
            elif "heat_combustion_mass:max":
                property_dict={"key":"heat_combustion_mass","value":val,"type":"max","unit":units}
...
def parseFile(path,inventory,group,api,experiment):
    ...
    file = pd.read_excel(path,header=[0,1])

    #Material objects are created for each row and added to inventory
    for index, row in file.iterrows():
        Material=cript.Material(group,row["Material_name"][0])
        add_identifiers(Material,row)
        add_properties(Material,row,group,api,experiment)
        api.save(Material)
...
if __name__ == "__main__":
    ...
    parseFile(config["path"],inventory,group,api,experiment)
